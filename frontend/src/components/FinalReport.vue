<template>
    <div class="space-y-6 animate-fade-up">
      ...
      <div class="bg-gray-800/50 border border-gray-700 rounded-2xl p-6">
        <p class="flex items-center gap-2 text-indigo-400 text-xs uppercase tracking-widest font-semibold mb-3">
          <Target :size="14" /> Conseil personnalisé
        </p>
        <p v-if="!advice" class="text-gray-400 italic flex items-center gap-2">
          <Loader2 :size="14" class="animate-spin" /> Analyse en cours...
        </p>
      ...
      <div class="bg-gray-800/50 border border-gray-700 rounded-2xl p-6">
        ...
        <div class="space-y-3">
          ...
          >
            <CheckCircle v-if="r.correct" :size="16" class="text-green-400 flex-shrink-0 mt-0.5" />
            <XCircle v-else :size="16" class="text-red-400 flex-shrink-0 mt-0.5" />
            <span class="text-gray-400 text-sm leading-relaxed">{{ r.question.question }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-center">
        <button
          class="flex items-center gap-2 px-10 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-full transition-all duration-200 hover:shadow-lg hover:shadow-indigo-500/25"
          @click="$emit('restart')"
        >
...
import { Target, Loader2, CheckCircle, XCircle, RotateCcw } from 'lucide-vue-next'
import { generateAdvice } from '../services/quizService.js'

const props = defineProps({ results: Array, config: Object })
defineEmits(['restart'])

const advice = ref('')

const correctCount = computed(() => props.results.filter(r => r.correct).length)
const scorePercent = computed(() => Math.round((correctCount.value / props.results.length) * 100))
...
onMounted(async () => {
  try {
    advice.value = await generateAdvice(props.results, props.config.difficulty, props.results.length)
  } catch {
...
<style scoped>
.animate-fade-up { animation: fadeUp 0.4s ease; }
@keyframes fadeUp {
